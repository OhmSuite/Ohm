...
/**
 * Can PID control a voltage controlled motor given a sensor. Can do either speed control or
 * position control.
 * 
 * Most importantly, this class will ensure that a given motor is only being used by one controller
 * at once, and that it has been given all required configuration before it runs.
 * 
 * @author Jacob Prinz
 */
public class PIDVoltageHardware {
	...
	public RangeOut<Speed> getSpeedOutput(RangeIn<Speed> speedSensor, PIDConstants config) {
		...
		return controller.getSetpointSetter().addChangeListener(newVal -> speedControlState.init());
	}
	...
	public RangeOut<Position> getPositionOutput(RangeIn<Position> positionSensor, PIDConstants config) {
		...
		return controller.getSetpointSetter().addChangeListener(newVal -> positionControlState.init());
	}
}
