...
public class CANTalonHardware extends Hardware<CAN>
{
	...
	/**
	 * @return a speed input stream indicating the current speed of the motor
	 *         based on the talon's preferred speed sensor <br>
	 *         uses a default range of [0,4096] specified by the CTRE mag
	 *         encoder, but may vary depending on the sensor
	 */
	...
	{
		wpiTalon.ifPresent(talon -> talon.setFeedbackDevice(device));
	}
	...
	{
		wpiTalon.get().setCurrentLimit(currentLimit);
	}
	 ...
	 *             if the talon failed to initialize due to port claiming
	 *             failure
	 */
